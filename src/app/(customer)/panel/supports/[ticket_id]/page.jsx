...
export default function CustomerTicketChat() {
  ...
  const fetchTicketAndReplies = async () => {
    try {
      const res = await axios.get(`/api/customer/supports/${ticket_id}/replies`);
      setTicket(res.data.ticket);
      setReplies(res.data.replies || []);
    } catch (err) {
      console.error('Failed to load ticket details', err);
      toast.error('Failed to load ticket details');
      router.push('/panel/supports');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSendReply = async (e) => {
    ...
    setSending(true);
    try {
      const res = await axios.post(`/api/customer/supports/${ticket_id}/replies`, {
        message: newMessage
      });
  ...
  if (loading) return <div className="text-center p-12 text-gray-500">Loading your ticket...</div>;
    ...
    <div className="flex flex-col h-[calc(100vh-140px)]">
      ...
      <div className="flex items-center justify-between mb-4 flex-shrink-0">
        <div className="flex items-center space-x-4">
          <Link href="/panel/supports" className="p-2 text-gray-500 hover:bg-gray-100 rounded-full transition-colors">
        ...
        <div>
          <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full capitalize ${ticket.status === 'open' ? 'bg-blue-900/50 text-blue-400' : ticket.status === 'in_progress' ? 'bg-yellow-900/50 text-yellow-400' : ticket.status === 'resolved' ? 'bg-green-900/50 text-green-400' : ' text-gray-400 border border-gray-200'} bg-white`}>
      ...
      {/* Chat Area */}
      <div className="flex-1 bg-white border border-gray-200 rounded-3xl overflow-hidden flex flex-col shadow-sm">
        {/* Messages List */}
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50/50">
          ...
          {replies.map((reply) => (
            <div key={reply.reply_id} className="flex flex-col mb-6 pb-6 border-b border-gray-200 last:border-0 last:pb-0 last:mb-0">
              <div className="flex items-center space-x-2 mb-2">
                <span className="font-bold text-sm text-gray-900">{!reply.is_staff ? 'You' : (reply.staff_name || 'Support Staff')}</span>
                {reply.is_staff && reply.staff_role && <span className="text-[10px] uppercase font-bold text-gray-600 bg-gray-100 px-1.5 py-0.5 rounded">{reply.staff_role}</span>}
        ...
        <div className="p-4 bg-white border-t border-gray-200">
          <form onSubmit={handleSendReply} className="flex space-x-4">
            <textarea
              ...
              rows="2"
              placeholder="Type your reply to support here..."
              value={newMessage}
          ...
          <div className="text-xs text-gray-400 mt-2 ml-1">
            {ticket.status === 'closed' ? 'This ticket is closed. You cannot reply anymore.' : 'Press Enter to send, Shift + Enter for new line.'}
          </div>
        </div>
      </div>
    </div>
