...
export default function GuideTours() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-white">Tour Catalog</h1>
        <p className="mt-1 text-gray-400">Review all active tour itineraries and details.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {tours.map((tour) => (
          <div key={tour.tour_id} className="bg-gray-800 rounded-3xl border border-[#222] p-6 hover:border-gray-600 transition-colors">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className={`px-2 py-1 text-xs font-bold rounded-full uppercase
                ${tour.status === 'active' ? 'bg-green-900/30 text-green-400 border border-green-500/20' : 
