        ...
        {tours.map((tour) => (
          <div key={tour.tour_id} className="rounded-3xl border border-gray-200 p-6 hover:border-gray-600 transition-colors bg-white">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className={`px-2 py-1 text-xs font-bold rounded-full uppercase ${tour.status === 'active' ? 'bg-green-900/30 text-green-400 border border-green-500/20' : ' text-gray-400'} bg-white`}>
                {tour.status}
              </span>
            </div>
