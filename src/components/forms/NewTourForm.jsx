...
export default function TourForm({ initialData = null, onSubmit, loading = false }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 max-w-4xl">
      <div className="bg-gray-800 p-8 rounded-3xl border border-[#222]">
        <h2 className="text-xl font-bold text-white mb-6">Tour Details</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-400 mb-2">Tour Title</label>
            <input
              type="text"
              name="title"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2">Base Location</label>
            <input
              type="text"
              name="location"
      ...
      <div className="bg-gray-800 p-8 rounded-3xl border border-[#222]">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-white">Tour Spots</h2>
          <button
            type="button"
            onClick={addSpot}
            className="px-4 py-2 bg-[#222] hover:bg-gray-700 text-white rounded-lg text-sm font-medium transition-colors border border-gray-700"
          >
            + Add Spot
          </button>
        </div>
        ...
        {spots.length === 0 ? (
          ...
          <div className="space-y-6">
            {spots.map((spot, index) => (
              <div key={index} className="p-6 bg-gray-900 rounded-2xl border border-gray-700 relative">
                <button
                  type="button"
                ...
                <h3 className="text-white font-medium mb-4">Spot {index + 1}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div className="flex items-center space-x-4">
                      {spot.image && (
                        <img src={spot.image} alt="Spot" className="w-16 h-16 rounded-xl object-cover border border-gray-700" />
                      )}
                      <div className="flex-1">
                        <input
                          ...
                          className="block w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-gray-700 file:text-white hover:file:bg-gray-600 disabled:opacity-50"
                        />
      ...
      <div className="flex justify-end gap-4">
        <button
          ...
          className="px-6 py-3 bg-transparent border border-gray-600 text-white font-medium rounded-xl hover:bg-gray-800 transition-colors"
        >
          Cancel
        </button>
        <button
